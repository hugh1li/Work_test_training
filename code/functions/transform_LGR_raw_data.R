...
cleanLGRdata <- function(x, tz_default = 'America/New_York') {
  ...
  data_re <- data_ref %>% mutate(DateTime =paste(Hem_GPS_Date, Hem_GPS_Time)) %>% select(DateTime , Lat, Lon, CH4, C2H2, H2O) %>% filter(!is.na(CH4)) %>% filter(CH4 > 1.5) %>% mutate(DateTime = lubridate::mdy_hms(DateTime, tz = tz_default)) %>% mutate(DateTime_adjusted = DateTime - seconds(50))
  data_re2 <- data_re %>% select(DateTime, Lat, Lon) %>% inner_join(data_re[c('DateTime_adjusted', 'CH4', 'C2H2', "H2O")], by = c('DateTime' = 'DateTime_adjusted'))
 return(data_re2)
}
